...
{
    struct alcor_sacfg_block_t* p_sacfg =
                         (struct alcor_sacfg_block_t*)dev->cfg->sacfg_base;
    struct alcor_nsacfg_block_t* p_nsacfg =
                       (struct alcor_nsacfg_block_t*)dev->cfg->nsacfg_base;

    switch(dev->cfg->ppc_name) {
        /* Case for MAIN0 */
        case PPC_ALCOR_MAIN0:
        ...
        case PPC_ALCOR_PERIPH_EXP3:
            ...
            break;
        default:
            return PPC_ALCOR_ERR_INVALID_PARAM;
        }

    dev->data->is_initialized = true;

    return PPC_ALCOR_ERR_NONE;
}
                            ...
                            enum ppc_alcor_priv_attr_t priv_attr)
{
    if(dev->data->is_initialized != true) {
        return PPC_ALCOR_ERR_NOT_INIT;
    }

    if(sec_attr == PPC_ALCOR_SECURE_ACCESS) {
#if (defined (__ARM_FEATURE_CMSE) && (__ARM_FEATURE_CMSE == 3U))
        /* Uses secure unprivileged access address (SACFG) to set privilege
         * attribute
         */
        if(priv_attr == PPC_ALCOR_PRIV_ONLY_ACCESS) {
        ...
        return PPC_ALCOR_ERR_NOT_PERMITTED;
#endif
    } else {
