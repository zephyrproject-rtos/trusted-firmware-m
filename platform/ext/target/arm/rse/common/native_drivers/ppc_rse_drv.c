...
{
    struct rse_sacfg_t* p_sacfg =
                         (struct rse_sacfg_t*)dev->cfg->sacfg_base;
    struct rse_nsacfg_t* p_nsacfg =
                       (struct rse_nsacfg_t*)dev->cfg->nsacfg_base;
    ...
    switch(dev->cfg->ppc_name) {
        ...
        case PPC_RSE_PERIPH_EXP3:
            ...
            break;
        default:
            return PPC_RSE_ERR_INVALID_PARAM;
        }

    dev->data->is_initialized = true;

    return PPC_RSE_ERR_NONE;
}
                            ...
                            enum ppc_rse_priv_attr_t priv_attr)
{
    if(dev->data->is_initialized != true) {
        return PPC_RSE_ERR_NOT_INIT;
    }

    if(sec_attr == PPC_RSE_SECURE_ACCESS) {
#if (defined (__ARM_FEATURE_CMSE) && (__ARM_FEATURE_CMSE == 3U))
        /* Uses secure unprivileged access address (SACFG) to set privilege
         * attribute
         */
        if(priv_attr == PPC_RSE_PRIV_ONLY_ACCESS) {
        ...
        return PPC_RSE_ERR_NOT_PERMITTED;
#endif
    } else {
