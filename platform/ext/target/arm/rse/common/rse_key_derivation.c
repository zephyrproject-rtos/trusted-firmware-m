...
static enum tfm_plat_err_t rse_get_boot_state(uint8_t *state, size_t state_buf_len,
                                              size_t *state_size)
{
    ...
    if (state_buf_len < 32) {
        return TFM_PLAT_ERR_KEY_DERIVATION_BOOT_STATE_BUFFER_TOO_SMALL;
    }
    ...
    cc_err = cc3xx_lowlevel_hash_init(CC3XX_HASH_ALG_SHA256);
    if (cc_err != CC3XX_ERR_SUCCESS) {
        return cc_err;
    }

    cc_err = cc3xx_lowlevel_hash_update((uint8_t *)&lcs, sizeof(lcs));
    if (cc_err != CC3XX_ERR_SUCCESS) {
        return cc_err;
    }

    cc_err = cc3xx_lowlevel_hash_update((uint8_t *)&tp_mode, sizeof(tp_mode));
    if (cc_err != CC3XX_ERR_SUCCESS) {
        return cc_err;
    }

    cc_err = cc3xx_lowlevel_hash_update((uint8_t *)&reprovisioning_bits, sizeof(reprovisioning_bits));
    if (cc_err != CC3XX_ERR_SUCCESS) {
        return cc_err;
    }

    cc_err = cc3xx_lowlevel_hash_update(computed_bl1_2_hash, 32);
    if (cc_err != CC3XX_ERR_SUCCESS) {
        return cc_err;
    }
...
static enum tfm_plat_err_t rse_derive_key(enum kmu_hardware_keyslot_t key_id, uint32_t *key_buf,
                                          ...
                                          bool duplicate_into_next_slot)
{
    ...
    plat_err = rse_get_boot_state(context, sizeof(context), &context_len);
    if (plat_err != TFM_PLAT_ERR_SUCCESS) {
        return plat_err;
    }
    ...
    cc_err = cc3xx_lowlevel_kdf_cmac(key_id, key_buf, CC3XX_AES_KEYSIZE_256, label,
                                         ...
                                         size_t *vhuk_seed_size)
{
    ...
    cc_err = cc3xx_lowlevel_kdf_cmac(KMU_HW_SLOT_HUK, NULL, CC3XX_AES_KEYSIZE_256,
                           ...
                           enum rse_kmu_slot_id_t slot)
{
    ...
    plat_err = rse_derive_key(KMU_HW_SLOT_GUK, NULL, ivs, ivs_len, slot, true);
    if (plat_err != TFM_PLAT_ERR_SUCCESS) {
        return plat_err;
    }
...
enum tfm_plat_err_t derive_using_krtl_or_zero_key(uint8_t *label, size_t label_len,
                                                  enum rse_kmu_slot_id_t output_slot)
{
    ...
    case LCM_TP_MODE_TCI:
        ...
        break;
    default:
        return TFM_PLAT_ERR_KEY_DERIVATION_INVALID_TP_MODE;
    }

    plat_err = rse_derive_key(input_slot, key_buf, label, label_len,
                              output_slot, true);
    if (plat_err != TFM_PLAT_ERR_SUCCESS) {
        return plat_err;
    }
