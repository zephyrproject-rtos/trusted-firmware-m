...
/*
 * MPC granularity is 32 KB on SSE300_MPS2 FVP. Alignment
...
#define S_CODE_START    ((SSRAM2_BASE_S) +  (S_IMAGE_PRIMARY_AREA_OFFSET))
#define S_CODE_SIZE     (IMAGE_S_CODE_SIZE - CMSE_VENEER_REGION_SIZE)
#define S_CODE_LIMIT    (S_CODE_START + S_CODE_SIZE - 1)

/* Secure Data stored in DTCM */
#define S_DATA_START    (DTCM_BASE_S)
#define S_DATA_SIZE     (DTCM_SIZE)
...
#define NS_CODE_START   (SSRAM2_BASE_NS + (NS_IMAGE_PRIMARY_AREA_OFFSET))
#define NS_CODE_SIZE    (IMAGE_NS_CODE_SIZE)
#define NS_CODE_LIMIT   (NS_CODE_START + NS_CODE_SIZE - 1)

/* Non-Secure Data stored in VM0 */
#define NS_DATA_START   (VM0_BASE_NS)
#define NS_DATA_SIZE    (VM0_SIZE)
#define NS_DATA_LIMIT   (NS_DATA_START + NS_DATA_SIZE - 1)

/* NS partition information is used for MPC and SAU configuration */
#define NS_PARTITION_START \
            ((SSRAM2_BASE_NS) + (NS_IMAGE_PRIMARY_PARTITION_OFFSET))
#define NS_PARTITION_SIZE (FLASH_NS_PARTITION_SIZE)

/* Secondary partition for new images in case of firmware upgrade */
#define SECONDARY_PARTITION_START \
            ((SSRAM2_BASE_NS) + (S_IMAGE_SECONDARY_PARTITION_OFFSET))
