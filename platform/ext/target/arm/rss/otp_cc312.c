...
static enum plat_otp_lcs_t dummy_lifecyle = PLAT_OTP_LCS_ASSEMBLY_AND_TEST;
...
static enum tfm_plat_err_t otp_read_lcs(size_t out_len, uint8_t *out) {
    if (out_len < sizeof(dummy_lifecyle)) {
        return TFM_PLAT_ERR_INVALID_INPUT;
    }

    memcpy(out, &dummy_lifecyle, sizeof(dummy_lifecyle));
